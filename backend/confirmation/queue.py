...
class ConfirmationQueue:
    def __init__(self, root: str = 'runtime/confirmations'):
        ...
        self._load()
    def _path(self, req_id: str) -> Path: return self.root / f'{req_id}.json'
    def _load(self):
    ...
    def _save(self, req: ConfirmationRequest): self._path(req.id).write_text(json.dumps(req.__dict__, ensure_ascii=False, indent=2), encoding='utf-8')
    def add(self, req: ConfirmationRequest) -> ConfirmationRequest:
        self.items[req.id] = req; self._save(req); return req
    ...
    def decide(self, req_id: str, approved: bool):
        req = self.items[req_id]; req.status = 'approved' if approved else 'rejected'; self._save(req); return req
