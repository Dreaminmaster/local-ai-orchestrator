...
class TaskPlanner:
    ...
    async def create_plan_from_optimized_context(self, optimized_context: dict) -> dict:
        fallback = self._fallback_plan_from_context(
            optimized_context, "local_model_fallback"
        )
    ...
    async def create_plan(self, goal: dict, state: dict | None = None) -> dict:
        context = {
            ...
            "authorization_contract": (state or {}).get("authorization_contract")
            or goal.get("authorization_contract")
            or {},
            "current_step": {},
    ...
    def _fallback_plan_from_context(self, context: dict, error: str) -> dict:
        ...
        if "read_files" in caps:
            skills.insert(0, "file")
            req.append("read_files")
        elif "operate_browser" in caps:
            skills.insert(0, "browser")
            req.append("operate_browser")
        elif "ask_external_ai" in caps:
            skills.insert(0, "external_ai")
            req.append("ask_external_ai")
        return {
            "plan": [
                {
                    "step": 1,
                    "goal": goal.get("final_goal") or "Execute next minimal step",
    ...
    def _fallback_plan(self, goal: dict, state: dict | None, error: str) -> dict:
        return self._fallback_plan_from_context(
            {
                "goal_contract": goal,
                "authorization_contract": (state or {}).get(
            ...
            steps.append(
                {
                    ...
                    "verification_method": "repair action produces evidence",
                    "failure_fallback": "escalate or ask user",
                    "risk_level": "medium" if caps else "low",
                    "can_auto_execute": True,
                }
            )
        return steps
