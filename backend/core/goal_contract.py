...
"""
        try:
            data = await self.llm.chat_json(
                [LLMMessage(role="user", content=prompt)], temperature=0.25
            )
        except Exception:
            data = {}
        return GoalContract(
            ...
            or (["自主补全目标和执行方案"] if goal_mode == "autonomous" else []),
            user_constraints=(user_constraints or [])
            + data.get("user_constraints", []),
            success_criteria=data.get("success_criteria")
            or ["完成用户目标", "有证据证明结果", "输出最终报告"],
            clarification_summary=data.get("clarification_summary")
            or clarification_answers,
            user_confirmed_goal=bool(
                data.get("user_confirmed_goal", goal_mode == "autonomous")
            ),
