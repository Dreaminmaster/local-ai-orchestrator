...
VERIFY_SCHEMA = {
    "verified": False,
    "criteria_results": [
        {"criterion": "...", "passed": False, "evidence_ids": [], "reason": "..."}
    ],
    "missing_items": [],
...
class Verifier:
    ...
    async def check_with_contracts(
        self,
        goal_contract: dict,
        ...
        context = {
            "goal_contract": {
                "final_goal": goal_contract.get("final_goal"),
                "success_criteria": criteria,
            },
            "authorization_contract": authorization_contract,
            "current_step": {"type": "final_verification"},
            "state_summary": json.dumps(
                {"result_summaries": results[-8:]}, ensure_ascii=False
            )[:2000],
    ...
    def _map_evidence(self, criteria: list, evidence: list[dict]) -> list[dict]:
        ...
        for c in criteria:
            tokens = [
                w
                for w in str(c).replace("，", " ").replace("。", " ").split()
                if len(w) > 1
            ]
            matches = []
            for ev in evidence:
    ...
    def _heuristic_verify(
        self, criteria: list, mapped: list[dict], results: list[dict]
    ) -> dict:
        criteria_results = []
        missing = []
        for item in mapped:
        ...
        verified = (
            bool(criteria) and not missing and any(r.get("success") for r in results)
        )
        return {
            "verified": verified,
            ...
            "reason": "heuristic verification",
        }

    async def check(
        self, goal: dict, results: list[dict], evidence: list[dict]
    ) -> dict:
        ...
        result = await self.check_with_contracts(
            goal_contract, authorization_contract, results, evidence
        )
        result.setdefault(
            "next_action", "finish" if result.get("verified") else "continue"
        )
        return result
