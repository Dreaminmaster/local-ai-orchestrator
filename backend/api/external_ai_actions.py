...
from backend.skills.external_ai_web.provider_status import ProviderState, normalize_provider
...
async def resume_pending_external_ai(task_id: str):
    ...
    if status.state in NEEDS_USER_STATES or status.state not in READY_STATES:
        artifact_store.update_state(
            task_id,
            status="needs_user",
            failure_reason=pending.get("failure_reason") or status.state.value.lower(),
            product_error=product_error(
                "EXTERNAL_AI_NEED_LOGIN",
                detail=pending.get("failure_reason") or status.state.value,
            ),
        ...
        return {
            ...
            "event": {
                ...
                "data": {"task_id": task_id, "provider": provider, "status": status.state.value},
            },
        }
    ...
    context.update(
        {
            ...
            "provider": provider,
            "target": provider,
            "prompt": pending.get("redacted_prompt") or pending.get("original_prompt", ""),
    ...
    out_path = artifact_store.task_dir(task_id) / "pending_external_ai_result.json"
    ...
    if result.success:
        ...
        artifact_store.update_state(
            ...
            product_error=product_error(
                "EXTERNAL_AI_NEED_LOGIN",
                detail=result.metadata.get("failure_reason") or result.error or "",
            ),
            pending_external_ai=True,
        )
    else:
    ...
    return {
        ...
        "event": {"type": event_type, "data": {"task_id": task_id, "provider": provider}},
    }
