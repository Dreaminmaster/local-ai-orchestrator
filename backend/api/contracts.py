...
from backend.core.clarification_session import (
...
async def prepare_goal(req: PrepareGoalRequest):
    ...
    if req.goal_mode == "clarify_first" and not req.clarification_answers:
        prepared = await ClarificationSessionService(llm).prepare(
            req.user_input, req.goal_mode
        )
        if prepared.get("needs_clarification"):
            return prepared
    contract = await GoalContractBuilder(llm).build(
        req.user_input, req.goal_mode, req.clarification_answers
    )
...
async def confirm_goal(req: ConfirmGoalRequest):
    ...
    if req.draft_goal_contract:
        data = req.draft_goal_contract.model_dump()
        data["user_confirmed_goal"] = True
        return data
    ...
    contract = await GoalContractBuilder(llm).build(
        session.original_input, session.goal_mode, summary
    )
    data = contract.model_dump()
    data["user_confirmed_goal"] = True
    data["clarification_summary"] = summary
    session.status = "contract_generated"
...
async def full_autonomy_preflight(req: PreflightRequest):
    return await DynamicPreflightAnalyzer(create_llm_provider()).analyze(
        req.user_input, req.goal_contract
    )
...
async def prepare_authorization(req: PrepareAuthorizationRequest):
    ...
    return (
        ...
        .build(
            ...
            req.provided_resources,
            caps,
            req.available_external_ai,
            req.protected_paths,
        )
        .model_dump()
    )
...
async def confirm_authorization(contract: AuthorizationContract):
    data = contract.model_dump()
    data["user_confirmed_authorization"] = True
    return data
...
async def start_task(req: StartTaskRequest):
    agent = Agent()
    events = []
    async for ev in agent.run_with_contracts(
        req.goal_contract.model_dump(), req.authorization_contract.model_dump()
    ):
