...
class CodeRepairLoop:
    def __init__(self): self.analyzer=ProjectAnalyzer(); self.diff=DiffManager(); self.adapter=CodexCLIAdapter()
    async def repair(self, issue: str, cwd: str = ".") -> dict:
        ...
        return {"project": project, "agent_result": result, "diff": self.diff.diff(cwd), "status": self.diff.status(cwd)}
