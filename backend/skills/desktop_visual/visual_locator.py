class VisualLocator:
    ...
    def locate(self, observation, description: str, hints: dict | None = None) -> dict:
        ...
        if self.vision_backend:
            try:
                located = self.vision_backend.locate(observation.screenshot_path, description)
                if located:
                    located.setdefault("method", "vision_backend")
                    return located
            except Exception as exc:
                return {"x": observation.width // 2, "y": observation.height // 2, "confidence": 0.1, "method": "vision_backend_failed", "error": str(exc)}
