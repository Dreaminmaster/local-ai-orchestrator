...
class DesktopVisualSkill(Skill):
    ...
    capabilities = ["observe_screen", "click_by_description", "type_text", "hotkey", "ocr_locate", "vision_locate", "ui_tars_reserved"]
    ...
    async def execute(self, instruction: str, context: dict) -> SkillResult:
        ...
        if action == "ocr_locate":
            found = self.ocr.locate_text(obs.screenshot_path, context.get("text", instruction))
            return SkillResult(self.name, bool(found), "OCR locate result", evidence=[obs.screenshot_path], metadata={"location": found})
        if action == "vision_locate":
            found = self.vision_backend.locate(obs.screenshot_path, context.get("target", instruction))
            return SkillResult(self.name, bool(found), "Vision locate result", evidence=[obs.screenshot_path], metadata={"location": found})
        if action == "click_by_description":
            ...
            after = self.observer.observe(context.get("after_save_as", "runtime/evidence/screen_after_click.png"))
            return SkillResult(self.name, True, f"Clicked target: {context.get('target', instruction)}", evidence=[obs.screenshot_path, after.screenshot_path], metadata={"location": loc, "action": res, "after": after.__dict__})
        if action == "type_text":
            res = self.executor.type_text(context.get("text", instruction))
            after = self.observer.observe(context.get("after_save_as", "runtime/evidence/screen_after_type.png"))
            return SkillResult(self.name, True, "Typed text", evidence=[obs.screenshot_path, after.screenshot_path], metadata=res)
        if action == "hotkey":
            return SkillResult(self.name, True, "Pressed hotkey", evidence=[obs.screenshot_path], metadata=self.executor.hotkey(context.get("keys", [])))
