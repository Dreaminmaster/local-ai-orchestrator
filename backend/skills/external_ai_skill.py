...
class ExternalAISkill(Skill):
    ...
    capabilities = [
        "ask_chatgpt", "ask_claude", "ask_deepseek", "ask_gemini",
        "ask_best_for", "compare_answers",
    ]
    risk_level = RiskLevel.LOW

    async def can_handle(self, task: dict, state: dict) -> bool:
        keywords = ["ask", "external", "chatgpt", "claude", "deepseek", "gemini", "expert", "opinion"]
    ...
    async def _ask_ai(self, target: str, question: str, context: dict) -> SkillResult:
        ...
        return SkillResult(skill=self.name, success=False, result="", error="Unsupported target")
    ...
    async def _call_openai_compatible(
        ...
        return SkillResult(
            skill=self.name, success=True, result=answer,
            metadata={"target": target_name, "model": model},
        )

    async def _call_anthropic(self, api_key: str, model: str, question: str) -> SkillResult:
        headers = {
            "x-api-key": api_key,
    ...
    async def _call_gemini(self, api_key: str, model: str, question: str) -> SkillResult:
    ...
    async def _compare(self, question: str, context: dict) -> SkillResult:
        ...
        return SkillResult(
            skill=self.name, success=True, result=comparison,
