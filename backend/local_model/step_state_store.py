...
class StepStateStore:
    ...
    def _path(self, task_id: str) -> Path:
        d=self.root/task_id; d.mkdir(parents=True, exist_ok=True); return d/'step_state.json'
    def save(self, state: StepState):
        self._path(state.task_id).write_text(json.dumps(state.__dict__, ensure_ascii=False, indent=2), encoding='utf-8')
    ...
    def list_resumable_tasks(self) -> list[StepState]:
        ...
        for p in self.root.glob('*/step_state.json'):
            try: out.append(StepState(**json.loads(p.read_text(encoding='utf-8'))))
