class ToolResultSummarizer:
    def summarize(self, skill_name: str, result: dict) -> dict:
        ...
        if skill_name == "shell":
            return self._summarize_shell(raw, result)
        if skill_name == "browser":
            return self._summarize_browser(raw, result)
        if skill_name in ["external_ai", "web_ai"]:
            return self._summarize_external_ai(raw, result)
        return {
            "skill": skill_name,
            "summary": raw[:1500],
    ...
    def _summarize_external_ai(self, raw: str, result: dict) -> dict:
        return {
            "skill": result.get("skill", "external_ai"),
            "summary": raw[:2500],
            "success": result.get("success", False),
