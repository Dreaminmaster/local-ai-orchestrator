...
class LocalModelRetryPolicy:
    ...
    async def run_with_retry(self, llm, build_prompt, parser, context, output_schema, fallback, escalation_router=None):
        ...
        for attempt in range(self.max_retries):
            ...
            attempts.append(raw); context=self._shrink_context(context)
        if escalation_router and escalation_router.should_escalate('json_parse_failed', {'json_parse_failures':len(attempts)}):
            return {'needs_escalation': True, 'reason':'Local model failed to produce valid JSON', 'attempts':[str(x) for x in attempts]}
        return fallback
    def _shrink_context(self, context: dict) -> dict:
        c=dict(context); c['relevant_evidence']=c.get('relevant_evidence',[])[:2]; c['state_summary']=c.get('state_summary','')[:800]; return c
