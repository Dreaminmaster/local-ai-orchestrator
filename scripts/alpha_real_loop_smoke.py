...
async def test_full_autonomy_chain():
    goal = {
        ...
        "original_input": "帮我检查这个仓库下一步还差什么",
        "final_goal": "检查仓库下一步缺口并输出报告",
        "assumptions": ["只做安全检查"],
        "success_criteria": ["生成计划", "保存证据", "生成报告"],
        "completion_standard": "有报告",
    }
    auth = {
        "authorization_mode": "full_autonomy",
        "granted_capabilities": ["read_files", "ask_external_ai", "take_screenshots"],
    ...
    async for ev in Agent().run_with_contracts(goal, auth):
        ...
        if len(events) > 20:
            break
    assert (
        "goal_contract" in events
        and "authorization_contract" in events
        and "plan" in events
    )
